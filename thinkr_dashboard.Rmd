...
=====================================
```{r issue_progress, include = FALSE}
# Keep only repo where we are owners
issue_data <- map_dfr(orgs, issue_progress, dashboard_start, privacy) %>% 
  distinct() %>% 
  filter(owner %in% orgs)
...
if (!is.null(pr_data)) {
  pr_data %>% 
    filter(!is.na(closed)) %>% 
    valueBox(nrow(.), icon = "fa-exclamation-circle")
} else {
